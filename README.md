...
最新更新：退出补报价优先处理到期持仓，在原每分钟 10 次预算内预留请求；记录脱敏 RPC 错误类别。默认 PAPER_PREBUY_FILTER=true：模拟买入直接拦截前15秒买入金额占比<20%、前60秒已跌超20%、砸单≥40 SOL，以及砸单前连续至少3笔卖出且前5秒净卖出的候选；任一命中即跳过。保留后台观察及新旧过滤研究组，不改变退出参数或实盘下单路径。部署与核对见 [观察实验说明](helius/OBSERVATION.md)。
