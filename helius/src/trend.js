...
class Trend{
 ...
 onSwap(s){const now=s.receivedAt;if(!this.connected||!Number.isFinite(now)||!Number.isFinite(s.eventTime)||Math.abs(now-s.eventTime)>5000||!Number.isFinite(s.price)||s.price<=0||!Number.isFinite(s.quoteSol)||s.quoteSol<0)return;
