...
function recoveryAudit(records, outcomes) {
  ...
  for (const r of records.values()) {
    ...
    const names = ['all', 'joint', 'highRebound', ...['prebuyLegacy', 'avoidConsecutivePressure', 'avoidWeakBuy', 'avoidPriorFall', 'avoidLargeDump', 'prebuyCombined', 'prebuyAllowUnknown', 'prebuyRequireKnown', 'prebuyUnknownOnly'].filter(n => r.selection?.arms?.[n])];
