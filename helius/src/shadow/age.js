'use strict';
class Age {
  constructor(entries = []) { this.pools = new Map(); for (const e of entries.slice(-20000)) this.created(e); }
  created(e) {
    if (!e || e.source !== 'pump_migrate_processed' || e.migrationAt !== e.createdAt || typeof e.pool !== 'string' || typeof e.mint !== 'string' || !Number.isSafeInteger(e.createdAt)
      || e.createdAt <= 0 || !Number.isSafeInteger(e.observedAt) || e.createdAt > e.observedAt + 2000) return false;
    ...
    if (this.pools.size > 20000) this.pools.delete(this.pools.keys().next().value);
    return true;
  }
  snapshot(s, at) {
