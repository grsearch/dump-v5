...
本版本提供运行日志和命令行测速，不提供网页面板、人工加币、竞品跟单、RSI、FDV、代币年龄和成交量筛选。
...
本次 55 项测试全部通过，包含新增样本时序、缺失标签、离线训练、模型校验、真实工作线程写盘及 COS 归档/重试测试；详见 [验证记录](VALIDATION.md)。发行 zip 包含独立运行所需代码、锁文件、说明和测试，不包含 node_modules、采集数据、训练模型或密钥。
