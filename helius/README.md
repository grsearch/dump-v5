...
新增风险/收益离线训练目标 `loss_25`、`net_return`，训练校准和测试均使用运行时的异常特征过滤。可分别配置观察模型，预测只落盘，不控制订单。默认新增 250ms / 1000ms 退出延迟及净收益 5% 提前止盈三组观察对照，自动进入每日 COS 归档；真实买卖阈值不变。配置、训练命令与结果解释见 [观察与训练说明](OBSERVATION.md)。
...
本次 82 项测试全部通过，包含新增样本时序、缺失标签、离线训练、模型校验、真实工作线程写盘及 COS 归档/重试测试；详见 [验证记录](VALIDATION.md)。发行 zip 包含独立运行所需代码、锁文件、说明和测试，不包含 node_modules、采集数据、训练模型或密钥。
