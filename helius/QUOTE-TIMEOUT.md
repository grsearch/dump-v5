...
最新实盘阈值（含校准）：固定止盈 10%，移动止盈激活 8%、相对最高价格回撤 3% 卖出，最长持仓 20 秒；固定止损关闭，行情超时仍为 10 秒。行情正常更新也会在持仓时钟 20 秒到期后启动退出。各条件先触发先退出，提交与确认可能晚于触发时刻。

启动与归档中核对 `strategyConfig.liveExitPolicy={version:1,takeProfit:10,trailArm:8,trailDrop:3,maxHoldMs:20000}`。这些实盘值独立于旧 .env 的 TAKE_PROFIT_PCT / TRAILING_ACTIVATE_PCT / MAX_HOLD_MS；旧参数继续用于纸面及 Shadow 研究，更新无需改旧 .env。不要把顶层研究参数误认成实盘有效参数。新持仓按提交日志 holdingStartedAt 计算 20 秒，旧持仓无此字段时沿用 openedAt，部署后超时仓可能立即开始退出；已提交交易仍先核对结果，不重复下单。
