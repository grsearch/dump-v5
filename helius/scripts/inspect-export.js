...
async function inspect(directory) {
  ...
  for (const target of ['rebound_30s', 'rebound_60s', 'strategy_proxy']) {
    ...
    for (const o of outcomes.values()) {
      ...
      if (!s?.features?.ready || !s.decisionFresh || !(s.features.lastHistorySequence < s.sequence)
    ...
    targets[target] = { censored, policies };
  }
  return { integrity: 'verified', lines, window: summary.window, snapshotAt: summary.snapshotAt, windowRecords: summary.stats.windowRecords,
    configSizeSol: summary.config?.sizeSol, sampleRecords: samples.size, counts, targets, dataQuality: summary.dataQuality,
