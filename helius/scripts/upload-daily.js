...
function reportConfig(env = process.env) {
  ...
  const prefix = env.COS_PREFIX || 'dump-sniper/daily', instance = env.COS_INSTANCE_ID || 'siliconvalley-01';
  ...
  return { bucket, region, prefix, instance, outputDir: path.resolve(__dirname, '..', env.COS_EXPORT_DIRECTORY || 'data/exports') };
}
